<%-- 
    Document   : ticketspercompany
    Created on : May 1, 2021, 9:09:37 PM
    Author     : Philip
--%>
...
<html lang="en">
    ...
    <body class="h-screen overflow-hidden flex items-center justify-start" style="background: #edf2f7">
        ...
        <%
            ...
            if (session.getAttribute("user") == null) {
                ...
                try {
                    ...
                    session.setAttribute("trigger", "ticketspercompany");
                    response.sendRedirect("login.jsp");
                }
                request.setAttribute("email", account.getEmail());
                request.setAttribute("role", account.getAccountRole());
                TicketDao ticketDao = new TicketDao();
                ticketlist = ticketDao.getCompanyTickets(account.getCompany().getId());
            }
        ...
        <div class="overflow-x-auto bg-white w-5/6 h-screen mt-0 md:mt-0 md:col-span-2">
            ...
            <h3 class="block text-base text-center font-medium text-gray-700 py-3"> All Tickets</h3>
            <table class="min-w-max w-full table-auto">
                <thead>
                    <tr class="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                        <th class="py-3 px-6 text-left">passenger</th>
                        <th class="py-3 px-6 text-left">source</th>
                        <th class="py-3 px-6 text-left">destination</th>
                        <th class="py-3 px-6 text-left">bought on</th>
                ...
                <tbody class="text-gray-600 text-sm font-light">
                    ...
                    <tr class="border-b border-gray-200 hover:bg-gray-100">
                        <td class="py-3 px-6 text-left whitespace-nowrap font-medium"><%=ticket.getPassenger().getFirstName()%> <%=ticket.getPassenger().getLastName()%></td>
                        <td class="py-3 px-6 text-left whitespace-nowrap font-medium"><%=ticket.getJourney().getRoute().getSource()%></td>
                        <td class="py-3 px-6 text-left whitespace-nowrap font-medium"><%=ticket.getJourney().getRoute().getDestination()%></td>
                        <td class="py-3 px-6 text-left whitespace-nowrap font-medium"><%=ticket.getBoughtOn()%></td>
                        <td class="py-3 px-6 text-left whitespace-nowrap font-medium"><%=ticket.getJourney().getPrice()%></td>
                        
                        <td>
                            <%
                                if (ticket.getTicketStatus().toString().equals("UNUSED")) {
                            %>
                            <span class="mx-auto bg-green-200 text-green-800 py-1 px-3 rounded-full text-xs">UNUSED</span>
                            <%} else {%>
                            <span class="mx-auto bg-red-200 text-red-800 py-1 px-3 rounded-full text-xs">USED</span>
                            <%}%>
                        </td>
                    </tr>
                    <%}%>
