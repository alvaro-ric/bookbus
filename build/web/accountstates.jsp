<%-- 
    Document   : accountstates
    Created on : May 2, 2021, 5:32:54 AM
    Author     : Philip
--%>
...
<%@page import="model.AccountState"%>
    ...
    <body class="h-screen overflow-hidden flex items-center justify-start" style="background: #edf2f7">
        ...
        <%
            ...
            if (session.getAttribute("user") == null) {
                response.sendRedirect("login.jsp?trigger=accountstates");
            } else {
            ...
            AccountDao<AccountState> accountDao = new AccountDao<>(AccountState.class);
            Long id = Long.parseLong(request.getParameter("id"));
            List<AccountState> companyStateList = accountDao.getAccountStates(id);
            //            request.setAttribute("genderlist", ds.getGenderList());
            ...
            <table class="min-w-max w-full table-auto">
                <thead>
                    <tr class="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                        <th class="py-3 px-6 text-left">account names</th>
                        <th class="py-3 px-6 text-left">email</th>
                        <th class="py-3 px-6 text-left">Company status</th>
                        <th class="py-3 px-6 text-left">updated On</th>
                        <th class="py-3 px-6 text-left">Done by</th>
                    </tr>
                </thead>
                <tbody class="text-gray-600 text-sm font-light">
                    <%
                        for (AccountState accounState : companyStateList) {

                    %>
                    <tr class="border-b border-gray-200 hover:bg-gray-100">
                        <td class="py-3 px-6 text-left whitespace-nowrap font-medium"><%=accounState.getManagerAccount().getManager().getFirstName()%> <%=accounState.getManagerAccount().getManager().getLastName()%></td>
                        <td class="py-3 px-6 text-left whitespace-nowrap font-medium"><%=accounState.getManagerAccount().getEmail()%></td>
                        <td>
                            <%
                                if (accounState.getAccountStatus().toString().equals("CREATED")) {
                            %>
                            <span class="mx-auto bg-yellow-200 text-yellow-800 py-1 px-3 rounded-full text-xs"><%=accounState.getAccountStatus()%></span>
                            <%} else if (accounState.getAccountStatus().toString().equals("APPROVED")) {%>
                            <span class="mx-auto bg-green-200 text-green-800 py-1 px-3 rounded-full text-xs"><%=accounState.getAccountStatus()%></span>
                            <%} else {%>
                            <span class="mx-auto bg-red-200 text-red-800 py-1 px-3 rounded-full text-xs"><%=accounState.getAccountStatus()%></span>
                            <%}%>
                        </td>
                        <td class="py-3 px-6 text-left whitespace-nowrap font-medium"><%=accounState.getUpdateOn()%></td>
                        <td class="py-3 px-6 text-left whitespace-nowrap font-medium"><%=accounState.getDoneBy().getFirstName()%> <%=accounState.getDoneBy().getLastName()%></td>
